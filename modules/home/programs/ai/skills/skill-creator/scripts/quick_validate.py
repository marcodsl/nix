...
try:
    # Resolved at runtime via the script's inline uv dependency metadata.
    import yaml  # pyright: ignore[reportMissingModuleSource]
except ImportError as exc:  # pragma: no cover - import guard for plain-python use
    raise SystemExit(
        "PyYAML is required for quick_validate.py. Run this script with "
        "`<path-to-skill-creator>/scripts/quick_validate.py ...` or install `pyyaml` "
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print(
            "Usage: <path-to-skill-creator>/scripts/quick_validate.py <skill_directory>"
        )
        sys.exit(1)
