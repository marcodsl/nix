                ...
                gaps.append(
                    {
                        ...
                        "remediation": (
                            f"Implement control(s) addressing {tsc_id}: {tsc_desc}"
                        ),
                    }
                )
            else:
                    ...
                    partial.append(
                        {
                            ...
                            "remediation": (
                                "Complete implementation and assign owners for"
                                f" {tsc_id} controls"
                            ),
                        }
                    )
            ...
            issues.append(
                {
                    ...
                    "severity": "critical",
                    "detail": (
                        f"Control status is '{ctrl.get('status', 'Not Started')}' "
                        "— operating evidence needed"
                    ),
                }
            )
...
def build_report(
    ...
    type2_gaps: list[dict[str, Any]],
) -> dict[str, Any]:
    """Build the complete gap analysis report."""
    total_criteria = sum(len(REQUIRED_TSC[c]) for c in categories if c in REQUIRED_TSC)
...
def format_text_report(report: dict[str, Any]) -> str:
    ...
    if gaps:
        ...
        for g in gaps:
            ...
            lines.append(f"  [{sev}] {g['tsc_criteria']}: {g['description']}")
    ...
    if "type2_operating_gaps" in report:
        ...
        ctrls_with = t2["controls_with_issues"]
        total_issues = t2["total_issues"]
        lines.append(
            f"--- Type II Operating Gaps ({ctrls_with} controls,"
        ...
        for detail in t2["details"]:
            ...
            for issue in detail["issues"]:
                ...
                lines.append(f"    - [{sev}] {issue['check']}: {issue['detail']}")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "SOC 2 Gap Analyzer — identifies gaps between current controls"
    ...
    parser.add_argument(
        ...
        required=True,
        help=(
            "Path to JSON file with current controls"
            " (from control_matrix_builder.py or custom)"
        ),
    )
    parser.add_argument(
        "--type",
        ...
        default="type1",
        help=(
            "Audit type: type1 (design only) or"
            " type2 (design + operating effectiveness)"
        ),
    )
    parser.add_argument(
        "--json",
