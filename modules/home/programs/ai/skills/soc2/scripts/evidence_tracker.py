...
def generate_status_report(controls: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    total = len(controls)
    status_counts = dict.fromkeys(EVIDENCE_STATUSES, 0)
    by_category: dict[str, dict[str, int]] = {}
    # Group issues by control_id so a single control with multiple problems
    # produces one entry in the report rather than one per issue.
    issues_by_control: dict[str, dict[str, Any]] = {}

    def _record_issue(ctrl: dict[str, Any], note: str, **extra: Any) -> None:
        cid = ctrl.get("control_id", "N/A")
        entry = issues_by_control.setdefault(
            cid,
            {
                "control_id": cid,
    ...
    for ctrl in controls:
        ...
        if category not in by_category:
            by_category[category] = dict.fromkeys(EVIDENCE_STATUSES, 0)
        ...
        if status == "overdue":
            ...
            _record_issue(ctrl, "Evidence collection not started")

        missing = [f for f in REQUIRED_FIELDS if f not in ctrl or not ctrl[f]]
        if missing:
            _record_issue(ctrl, f"Missing fields: {', '.join(missing)}")

    issues = list(issues_by_control.values())
...
def format_status_text(report: dict[str, Any]) -> str:
    ...
    rating = summary["readiness_rating"]
    lines.append(f"Readiness Score: {score}% ({rating})")
    lines.append("")
    ...
    for cat, statuses in report["by_category"].items():
        ...
        cat_applicable = cat_total - statuses.get("not_applicable", 0)
        cat_collected = statuses.get("collected", 0)
        cat_pct = (
            round(cat_collected / cat_applicable * 100, 1) if cat_applicable > 0 else 0
        )
        lines.append(
            f"  {cat}: {cat_collected}/{cat_applicable} collected ({cat_pct}%)"
        )
    lines.append("")
    ...
    if report["issues"]:
        ...
        for entry in report["issues"]:
            ctrl_id = entry.get("control_id", "N/A")
            for note in entry.get("issues", []):
                lines.append(f"  [{ctrl_id}] {note}")
    else:
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "SOC 2 Evidence Tracker — tracks evidence collection status per control."
        )
    )
    parser.add_argument(
        "--matrix",
