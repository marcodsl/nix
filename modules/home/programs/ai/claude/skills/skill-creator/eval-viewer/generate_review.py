...
def _load_metadata_prompt(run_dir: Path) -> tuple[str, int | None]:
    """Try eval_metadata.json files for prompt and eval_id."""
    prompt = ""
    eval_id = None
    for candidate in [
...
def _load_transcript_prompt(run_dir: Path) -> str:
    """Fall back to transcript.md for prompt."""
    for candidate in [
...
def _collect_output_files(outputs_dir: Path) -> list[dict]:
    """Embed all non-metadata output files in directory."""
...
def _load_grading(run_dir: Path) -> dict | None:
    """Load grading.json from run dir or its parent."""
...
def build_run(root: Path, run_dir: Path) -> dict | None:
    ...
    prompt, eval_id = _load_metadata_prompt(run_dir)
    if not prompt:
        prompt = _load_transcript_prompt(run_dir)
    if not prompt:
...
def _error_file(path: Path) -> dict:
    return {"name": path.name, "type": "error", "content": "(Error reading file)"}


def _read_b64(path: Path) -> str | None:
    """Return base64 of file bytes, or None on read error."""
    try:
