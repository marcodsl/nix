...
Use this file as the prompt scaffold for a background `general-purpose` agent.
Provide the concrete benchmark or comparison paths in the task prompt and let
that agent write the analysis output JSON.
...
4. If the comparator flagged an invalid or contaminated comparison, carry that
   caveat forward. Do not overstate improvement suggestions from a benchmark
   that was not a clean A/B test.
...
2. Note the configurations tested (for example `with_skill`, `without_skill`, or snapshot-vs-snapshot)
3. Understand the run_summary aggregates already calculated
4. Inspect any per-run validity metadata available in the benchmark or adjacent
   `timing.json` / `metrics.json` files, especially `skill_triggered` and
...
- Are supposed baseline failures or wins actually coming from contaminated runs?
...
- Did a "without_skill" run still trigger the target skill, making the comparison less trustworthy?
...
- "Eval 1's without-skill run still triggered the target skill, so treat the apparent baseline as contaminated"
...
- Flag contaminated baseline runs before drawing conclusions from them
...
- Treat contaminated baselines as clean evidence for or against the skill
